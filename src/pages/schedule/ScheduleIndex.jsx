...
const ScheduleIndex = () => {
  ...
  // 여행 확인하기
  const getTrip = async () => {
    const sendData = { trip_id: selectedTripId };
    try {
      const res = await jwtAxios.get(
        `/api/trip?trip_id=${selectedTripId}`,
        sendData,
      );
    ...
    <div>
      ...
      <div className="flex flex-col gap-[30px] py-[30px]">
        ...
        <div className="mt-[60px] flex flex-col gap-[10px] px-[32px]">
          ...
          <h2 className="text-[36px] text-slate-700 font-bold">타이틀</h2>
        </div>
        {/* 버튼 */}
        ...
        <div>
          {dummyDays?.map((item, index) => {
            return <ScheduleDay newTrip={true} data={item} key={index} />;
          })}
        </div>
      </div>
    </div>
